...
$titre_forum = ($tab_forum[0]["no_forum_type"]==1)?"Forum citoyen de ".ucfirst(strtolower($titre_ville))." !":$tab_forum[0]["titre"];
$premier_message = ($tab_forum[0]["no_forum_type"]==1)?"Vie locale, informations à partager, actions collectives, idées...<br />Un espace neutre et indépendant pour échanger en liberté !<br /><br />Cet espace de libre expression est ouvert à tous, dans le respect de quelques règles élémentaires:<br /><br /><ol><li>Tout propos puni par la loi, discriminatoire ou diffamatoire sera supprimé.</li><li>Les publicités commerciales sont interdites.</li><li>La courtoisie est de rigueur : toute critique doit se faire dans le respect de la personne.</li><li>Tout acte de modération sera justifié par un message privé envoyé à l’auteur.</li></ol>":$tab_forum[0]["contenu"];
...
?>
<a href="<?php echo $root_site.$titre_ville_url.".".$id_ville.".tout.forums.html"; ?>" style="font-size:1.2em;color: rgb(45, 171, 218);text-shadow: 0px 0px 1px rgb(229, 238, 242);" onmouseover="this.style.backgroundColor='rgb(45, 171, 218)';this.style.color='white';" onmouseout="this.style.color='rgb(45, 171, 218)';this.style.backgroundColor='transparent';">&lt;&lt;&nbsp;Retour aux forums</a>
<h1 style="color:rgb(21,170,158);"><?php echo $titre_forum ?></h1>
<div class="zone_question">
	<div class="contenu_question">
		<?php
		if($tab_forum[0]["no_forum_type"]!=1){
		?>
		<div class="illustr">
			<img src="<?php echo $root_site; ?>miniature.php?uri=<?php echo $tab_forum[0]['url_image']; ?>&amp;method=fit&amp;w=150" width="150" />
		</div>
		<?php
		}
		echo $premier_message; ?>
	</div>
</div>
...
for($i=0;$i<count($tab_message);$i++){
	...
	<div class="un_message" id="message_<?php echo $tab_message[$i]["no"]; ?>_<?php echo $tab_message[$i]["no_utilisateur"]; ?>">
		<div class="information_message">
			...
			<span class="pseudo"><?php echo $tab_message[$i]["utilisateur"]; ?></span> le <?php echo datefr($tab_message[$i]["date_modification"],true,false); ?>
			<?php
			if(est_connecte()){
		...
		<?php
			...
			?>
				<div class="un_commentaire" id="unCommentaire_<?php echo $tab_sous_message[$j]["no"]; ?>_<?php echo $tab_sous_message[$j]["no_utilisateur"]; ?>">
					...
					<div class="signature_commentaire"><a class="pseudo" href="profil.php?no=<?php echo $tab_sous_message[$j]["no_utilisateur"]; ?>"><?php echo $tab_sous_message[$j]["utilisateur"]; ?></a> le <?php echo datefr($tab_message[$i]["date_modification"],true,false); ?></div>
					<?php
						if(est_connecte()){
