...
if($count_cores === 1){
	...
	$StrQueryDC = "INSERT INTO `$table_derniere_connexion`
		    VALUES ('',:maintenant,:ip,:loginbdd,:user_agent)";
	...
	$QueryDC->bindParam(":user_agent", $_SERVER['HTTP_USER_AGENT'], PDO::PARAM_STR);
	$QueryDC->execute();
	$QueryDC->closeCursor();
	$QueryDC = NULL;
	...
	if (isset($url))
	{
		$page = $url."?action=ajoutfav";
		//header("Location: $url?action=ajoutfav");
		//exit;	
	}
	else
	{
		// Redirection vers la page (du domaine) demandée initialement (dans connexion_verif.php)
		if ( isset($_SESSION['connexion_redirect_page']) && (!empty($_SESSION['connexion_redirect_page'])) && (substr($_SESSION['connexion_redirect_page'], 0, strlen($root_site)) == $root_site) )
		{
				$page = $_SESSION['connexion_redirect_page'];
		...
		else
		{
			...
			if ($estAdmin)
				$page = $root_site."gestion/accueil_admin.php";
			elseif ($estEditeur)
				$page = $root_site."gestion/accueil_editeur.php";
			else
				$page = $root_site."espace_personnel.php";		
		}
	}
	if(!$ajax)
...
{
//Si le nombre de correspondances n'est pas égal à 1, redirection vers l'identification

	$page = 'inscription.php?etape=etape1&erreur=couple';
	if(!$ajax)
		header("Location: $root_site$page");
	else{
		if($_GET["not_echo"]!=1)
