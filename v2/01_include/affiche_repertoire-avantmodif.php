...
if (($id_ville) && (!empty($id_ville)))
{
    ...
    else if ($page_home) 
    {
      ...
      echo '<div class="titre">Répertoire</div>';
    ...
    else
    {
        ...
        // Page évènement sélectionnée, pour ajouter au lien rewritting
        $apage = intval($_GET["apage"]);
        if ($apage)
            $page_evt = ".ap-" . $apage;
        else
            $page_evt = "";
        ...
        if (!empty($id_tag))
        {
            ...
            $lien_pagination_url = $titre_ville_url . "." . url_rewrite($titre_nomtag) . ".tag." . $id_ville . "." . $id_tag . $url_id_vie . $page_evt . ".rp-";
            $lien_pagination_alt = $titre_nomtag . " à " . $titre_ville . ", page ";
            // url de destination des forms de filtre / tri (sans pagination)
            $lien_filtre_tri     = $titre_ville_url . "." . url_rewrite($titre_nomtag) . ".tag." . $id_ville . "." . $id_tag . $page_evt . ".html#repertoire";
            // Pour affichage
        ...
        elseif (!empty($id_vie))
        {
           ...
           FROM (
            ...
            $nb_pages            = ceil($nb_struct_total / $nb_struct_list);
            // Préparation du lien pour pagination
            $lien_pagination_url = $titre_ville_url . "." . url_rewrite($nom_url_vie) . "." . $id_ville . "." . $id_vie . $page_evt . ".rp-";
            $lien_pagination_alt = $titre_nomvie . " à " . $titre_ville . ", page ";
            // url de destination des forms de filtre / tri (sans pagination)
            $lien_filtre_tri     = $titre_ville_url . "." . url_rewrite($nom_url_vie) . "." . $id_ville . "." . $id_vie . $page_evt . ".html#repertoire";
            // Pour affichage
            $sql_struct          = "SELECT * FROM (($sql_struct) UNION ($sql_proche)) AS tmp GROUP BY `NumStructure` ORDER BY `Distance`,`nom`" . $limit;
            $res_struct          = $connexion->prepare($sql_struct);
            $res_struct->execute(array(
        ...
        else
        {
            ...
            if ($tous_struct)
            {
                $lien_pagination_url = $titre_ville_url . "." . $id_ville . ".tout.repertoire.rp-";
            ...
            {
                // Home
                $lien_pagination_url = $titre_ville_url . "." . $id_ville . $page_evt . ".rp-";
                $lien_pagination_alt = " Structures à " . $titre_ville . ", page ";
                $limit               = " LIMIT 0, $nb_struct_home";
                // url de destination des forms de filtre / tri (sans pagination)
                $lien_filtre_tri     = $titre_ville_url . "." . $id_ville . $page_evt . ".html#repertoire";
            }
            // Pour affichage
            $sql_struct = "SELECT * FROM (($sql_struct) UNION ($sql_proche)) AS tmp GROUP BY `NumStructure` ORDER BY `Distance`,`nom`" . $limit;
            $res_struct = $connexion->prepare($sql_struct);
        ...
        if ((!$espace_perso) && (!$page_home))
        {
            ...
            if (($nb_pages) && ($nb_struct_total > $nb_struct_list))
            {
                ...
                if ($rpage > 1)
                {
                    // page précédente <
                    $prevpage = $rpage - 1;
                    $rpagination .= "<a href='" . $lien_pagination_url . $prevpage . ".html#repertoire'><</a>";
                    // Lien page 1
                    if ($rpage > $nb_pages_aff + 1)
                        $rpagination .= "<a href='" . $lien_pagination_url . "1.html#repertoire'>1</a>";
                }
                ...
                for ($x = ($rpage - $nb_pages_aff); $x < (($rpage + $nb_pages_aff) + 1); $x++)
                {
                    if (($x > 0) && ($x <= $nb_pages))
                    {
                        ...
                        if ($x == $rpage)
                            ...
                            $rpagination .= "<a href=\"" . $lien_pagination_url . $x . ".html#repertoire\" title=\"" . $lien_pagination_alt . $x . "\">" . $x . "</a>";
                    }
                }
                ...
                if ($rpage != $nb_pages)
                {
                    ...
                    if ($nb_pages - $rpage > $nb_pages_aff)
                        $rpagination .= "<a href='" . $lien_pagination_url . $nb_pages . ".html#repertoire'>$nb_pages</a>";
                    $rpagination .= "<a href='" . $lien_pagination_url . $nextpage . ".html#repertoire'>></a>";
                }
                $rpagination .= "</div>";
                echo $rpagination;
            }
