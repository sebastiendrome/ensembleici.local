<?php
	...
	$meta_description = "Inscrire un ami à la lettre d'information Ensemble ici : Tous acteurs de la vie locale";
	...
	if((intval($_POST['doinscrireami']))==1)
	{
		...
		if ( (isset($_REQUEST['userCaptchaCode'])) && (!empty($_REQUEST['userCaptchaCode'])) )
		{
			...
			else
			{
				...
				// email déjà inscrit ?
				if ( (count($tab_utilisateur)) || ($existe_util_news) )
				{ 
					if ( ($existe_util_news) && (!$existe_util_news_etat) )
						$msg_deja = "Cette adresse email fait déjà parti des inscrits à la lettre d'information Ensemble ici mais son propriéraire s'est désabonné.<br/>Pour réinscrire cette adresse, veuillez créer un compte.";
					else
		...
		<form id="EDinscami" name="EDinscami" action="<?php echo $PHP_SELF;?>" method="post" accept-charset="UTF-8" class="formA">
			...
			<div class="captcha">
				...
				<ul>
					...
					<li>
						<label for="mail">Adresse email de votre ami :</label>
