...
if (($id_ville) && (!empty($id_ville)))
{
    ...
    else
    {
        ...
        if (!empty($id_tag))
        {
          ...
          $sql_pa = "SELECT P.no AS NumPa, P.etat, P.titre, P.monetaire, P.prix, P.url_image, P.rayonmax, P.date_fin, I.latitude, I.longitude, I.nom_ville_maj, 0 AS Distance
                    FROM  `petiteannonce` P,
                          `petiteannonce_tag` O,
                          `tag` T,
                          `villes` I
          ...
          $sql_proche = "SELECT *, 
            ...
            FROM (
              SELECT P.no AS NumPa, P.etat, P.titre, P.monetaire, P.prix, P.url_image, P.rayonmax, P.date_fin, I.latitude, I.longitude, I.nom_ville_maj
                      FROM  `petiteannonce` P,
                            `petiteannonce_tag` O,
                            `tag` T,
                            `villes` I
            ...
            // Pour affichage
            $sql_pa          = "SELECT * FROM (($sql_pa) UNION ($sql_proche)) AS tmp GROUP BY `NumPa` ORDER BY `Distance`,`titre`" . $limit;
            $res_pa          = $connexion->prepare($sql_pa);
            $res_pa->execute(array(
                ':id_tag' => $id_tag,
                ':id_ville' => $id_ville
        ...
        {
            // DEFAUT : Affichage sur Home + Toutes les petites annonces
          ...
          /*  $sql_proche        = "SELECT *, 
                          acos(sin($lat_ville_rep)*sin(radians(latitude)) + cos($lat_ville_rep)*cos(radians(latitude))*cos(radians(longitude)-$lon_ville_rep))*$rayon_terre AS Distance
                      FROM (
                        SELECT P.no AS NumPa, P.etat, P.titre, P.monetaire, P.prix, P.url_image, P.rayonmax, P.date_fin, latitude, longitude, nom_ville_maj
                        FROM  `petiteannonce` P,
                              `petiteannonce_tag` O,
                              `villes` V
                                WHERE P.etat = 1
