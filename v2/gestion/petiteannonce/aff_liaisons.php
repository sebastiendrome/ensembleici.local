...
if (($id_source)&&($type_source))
{
  ...
  if (count($tab_liaison))
  {
	...
	$liaison_nom = $liaison_nom["titre"];
      else
	$liaison_nom = $liaison_nom["nom"];
      
      // Affiche le nom de la liaison
      if ($liaison_nom)
	echo "<a href=\"..".$url_lien."modifajout.php?id=".$no_liaison."\" target=\"_blank\">".$liaison_nom."</a> <sup>(".$no_liaison.")</sup>";
