...
if($is_no){
	$mot=$_POST["m"];
	$cond = " WHERE structure.no REGEXP '".$mot."'";
}
else{
	$les_mots_cles = array();
...
//				$mot_cle = preg_replace('#[c�]#i', '([c�]|(&ccedil;))', $mot_cle); //(&ccedil;)
				...
				if($cond=="")
					...
					$cond .= " AND ";
				$cond .= "( structure.nom REGEXP '^".$mot_cle."' OR structure.nom REGEXP ' ".$mot_cle."' )";
				$les_mots_cles[] = $mot_cle;
		}
	}
}
$requete = "SELECT structure.no, structure.nom, villes.nom_ville AS ville FROM structure JOIN villes ON villes.id = structure.no_ville ".$cond." AND structure.etat=1 LIMIT 8";
...
if(count($tab_requete)>0){
	for($i=0;$i<count($tab_requete);$i++){
		...
		if($_POST["n"]==1)
			$reponse[] = array($tab_requete[$i]["no"],"<div style=\"display:inline;color:#2E2E2E;font-size:14px;\">".surligne_recherche($tab_requete[$i]["no"],array($mot))."</div> : <div style=\"display:inline;font-size:14px;font-weight:bold;color:#575757;\">".html_entity_decode($tab_requete[$i]["nom"])."</div>&nbsp;&nbsp;<div style=\"display:inline;color:#4E4F4F;font-size:12px;\">(".$tab_requete[$i]["ville"].")</div>");
		else
			$reponse[] = array($tab_requete[$i]["no"],"<div style=\"display:inline;color:#2E2E2E;font-size:14px;\">".$tab_requete[$i]["no"]."</div> : <div style=\"display:inline;font-size:14px;font-weight:bold;color:#575757;\">".surligne_recherche(html_entity_decode($tab_requete[$i]["nom"]),$les_mots_cles,$mot)."</div>&nbsp;&nbsp;<div style=\"display:inline;color:#4E4F4F;font-size:12px;\">(".$tab_requete[$i]["ville"].")</div>");
	}
}
else{
	$reponse[0] = false;
