...
if(isset($_POST["no"])&&!empty($_POST["no"])&&isset($_POST["no_l"])&&!empty($_POST["no_l"])){
	...
	//On récupère les liste
		if($etape=="repertoire"||$etape=="structure")
			$requete_liste = "SELECT liste_structure_complete AS l_c,liste_structure AS l FROM lettreinfo_repertoire WHERE no_lettre=:no_l";
			...
			$requete_liste = "SELECT liste_evenement_complete AS l_c,liste_evenement AS l FROM lettreinfo_agenda WHERE no_lettre=:no_l";
	...
	if($etape=="repertoire"||$etape=="structure")
		...
		$requete_update = "UPDATE lettreinfo_agenda SET liste_evenement_complete=:l_c, liste_evenement=:l WHERE no_lettre=:no";
