...
?>
    ...
    <h3>Ajouter une petite annonce</h3>
...
// function vide(e){if(e.firstChild){e.removeChild(e.firstChild);vide(e);}}
	$(function() { 
		$('#submit').click(function() {
			$('#form-ajout-petiteannonce').hide(0);
			var formData = $('form#form-ajout-petiteannonce').serialize();
			$.ajax({
				type : 'POST',
				url : 'doajoutpetiteannonce.php',
				dataType : 'json',
				data: formData,
...
function list_results(xhr, keyword)
{
	...
	$('#ajoutpetiteannonce').colorbox.resize();
...
function recherche(input){
	...
	XHR_SAM.open("POST", "ajax/recherche_petiteannonce.php", true);
...
<fieldset>
<label class="labellarge" for="input">Nom ou num&eacute;ro de la petite annonce</label>&nbsp;:&nbsp;<input class="input" id="input" name="input" type="text" size="35" onkeyup="recherche(this)" />
