$(function() {
    if ($('#url_don').html() != '') {
        if ($('#cookie_bandeau').html() == 0) {
            setTimeout(function(){
                $('#disclaimer').fadeIn(1000); 
//                alert('on affiche le bandeau');
            },2000);
        }
    }
    $('a[name=link_don]').on('click', function (e) {
        var post = {};
        var url = $('#base_url').html() + '03_ajax/register_cookie.php';
        $.post(url, post, function(reponse){
            $('#disclaimer').fadeOut(1000);
            $('#cookie_bandeau').html('1');
        });
    });
    $('#close_dislaimer').on('click', function (e) {
        e.preventDefault();
        var post = {};
        var url = $('#base_url').html() + '03_ajax/register_cookie.php';
        $.post(url, post, function(reponse){
    ...
    if ($('#reinit_mdp').html() != '') {
        ...
        $.post(url, post, function(reponse){
            if (reponse.code == 0) {
                ...
                if (reponse.code == 1) {
                    $('#body_mod_infos').html("Impossible de réinitialiser le mot de passe. Veuillez contacter l'administrateur du site.");
                    $('#modal_infos').modal(); 
                }
            }
        }, 'json');
...
//    $(document).on('click', '#show_tags', function(e) {
//        e.preventDefault();
//        if ($(this).attr('class') == 'ico-down') {
//            $(this).attr('class', 'ico-up');
//            $('#liste_tag').slideDown(1000);
//        }
//        else {
//            $(this).attr('class', 'ico-down');
//            $('#liste_tag').slideUp(1000);
//        }
//    });
    $(document).on('click', '#show_tags', function(e) {
        e.preventDefault();
        if ($(this).attr('class') == 'ico-plus-down') {
            $(this).attr('class', 'ico-plus-up');
            $(this).html('-');
            $('#liste_tag').slideDown(1000);
        }
        else {
            $(this).attr('class', 'ico-plus-down');
            $(this).html('+');
            $('#liste_tag').slideUp(1000);
        }
    });
    
    $(document).on('click', 'a[name=duplication_fiche]', function(e) {
        e.preventDefault();
        $('#duplicate_fiche_ref').html($(this).data('ref'));
        $('#duplicate_fiche_page').html($(this).data('page'));
        $('#duplicate_fiche_url').html($(this).data('url'));
        $('#modal_duplicate_fiche').modal('show');
    });
    
    $('#btn_valid_duplicate_fiche').on('click', function(e) {
        e.preventDefault();
        var ref = $('#duplicate_fiche_ref').html();
        var page = $('#duplicate_fiche_page').html();
        var url_page = $('#duplicate_fiche_url').html();
        var post = {
            ref: ref
        };
        var url = $('#base_url').html() + '03_ajax/utilisateurs/dupliquer_fiche.php';
        $.post(url, post, function(reponse){
            if (reponse.code == 0) {
                url_page += reponse.no + '.generalites.html';
                $(location).attr('href', url_page);
            }
            else {
                $('#modal_duplicate_fiche').modal('hide');
                if (reponse.code == '10') {
                    $('#body_mod_infos').html("Vous devez être connecté pour dupliquer une fiche");
                    $('#modal_infos').modal();
                }
                else {
                    $('#body_mod_infos').html("La fiche n'a pas pu être dupliquée.");
                    $('#modal_infos').modal();
                }
            }
        }, 'json');
