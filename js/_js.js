...
//                        element("update_infos_ville").value = libelle_cp;
			...
			xhr.send("no="+no);
                        if (xhr.responseText != '') {
                            element('lien_facebook').href = xhr.responseText;
                        }
			...
			if(inputs[i].type!="checkbox"){
				...
				if(inputs[i].name.replace("BDD","")=="pseudo"&&inputs[i].value!=""&&inputs[i].value!=inputs[i].title)
					pseudo_modifie = inputs[i].value;
                                    if ((inputs[i].name.replace("BDD","") == "site") && (inputs[i].value != " ")) {
	...
	if(analyser_ajax(reponse)){
		...
		return false;
            ...
            if (presence == 1) {
                ...
                if (presence == 2) {
                    $('#body_mod_infos').html("L'url de votre site web ne doit pas contenir le signe @. Si vous voulez ajouter une adresse email, utilisez l'ajout de contact ci-dessous.");
                    $('#modal_infos').modal();
                    return false;
                }
            }
