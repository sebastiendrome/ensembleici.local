...
if (sizeof($tab) > 0) {
    ...
    foreach ($tab as $k => $v) {
        ...
        if (isset($tab_lettre[0]['territoires_id'])) {
            ...
            $requete_utilisateur = "SELECT U.no, U.email, U.code_desinscription_nl as c, 'utilisateur' as type, U.no_ville FROM utilisateur U, communautecommune_ville V, communautecommune C WHERE U.newsletter = 1 
...
//            $tab_liste_envoi[2] = array('no' => 2, 'email' => 'contact@envolinfo.com', 'c' => 'aa', 'type' => 'utilisateur', 'no_ville' => 9568);
        ...
        if (isset($tab_agenda[0]['liste_evenement'])) {
            ...
            foreach ($tab_liste_agenda as $k2 => $v2) {
                // on regarde si l'événement est lié au territoire 
                $req_select_evenement = "SELECT C.territoires_id FROM evenement E LEFT JOIN communautecommune_ville V ON V.no_ville = E.no_ville 
                    LEFT JOIN communautecommune C ON C.no = V.no_communautecommune  WHERE E.no = :no";
                $tab_select_evenement = execute_requete($req_select_evenement, array(":no" => $v2));
                
                if (isset($tab_select_evenement[0]['territoires_id']) && ($tab_select_evenement[0]['territoires_id'] == $territoire)) {
        ...
        if (isset($tab_annonces[0]['liste_petiteannonce'])) {
            ...
            foreach ($tab_liste_annonces as $k2 => $v2) {
                // on regarde si l'annonce est liée au territoire 
                $req_select_annonces = "SELECT C.territoires_id FROM petiteannonce E LEFT JOIN communautecommune_ville V ON V.no_ville = E.no_ville 
                    LEFT JOIN communautecommune C ON C.no = V.no_communautecommune  WHERE E.no = :no";
                $tab_select_annonces = execute_requete($req_select_annonces, array(":no" => $v2));
                
                if (isset($tab_select_annonces[0]['territoires_id']) && ($tab_select_annonces[0]['territoires_id'] == $territoire)) {
        ...
        if (isset($tab_structure[0]['liste_structure'])) {
            ...
            foreach ($tab_liste_structure as $k2 => $v2) {
                // on regarde si la structure est liée au territoire 
                $req_select_structures = "SELECT C.territoires_id FROM structure E LEFT JOIN communautecommune_ville V ON V.no_ville = E.no_ville 
                    LEFT JOIN communautecommune C ON C.no = V.no_communautecommune  WHERE E.no = :no";
                $tab_select_structures = execute_requete($req_select_structures, array(":no" => $v2));
                
                if (isset($tab_select_structures[0]['territoires_id']) && ($tab_select_structures[0]['territoires_id'] == $territoire)) {
