...
$compl_req = '';
if (sizeof($tab_info_agenda) > 0) {
    // lettre à modifier
    $compl_req .= " AND E.no IN (".$tab_info_agenda[0]['liste_evenement_complete'].")";
    ...
    $requete_liste = "SELECT E.no, E.titre, E.url_image, E.date_debut, V.nom_ville, V.nom_ville_url, E.no_ville, G.libelle, E.apparition_lettre FROM evenement E, communautecommune_ville T, communautecommune C, villes V, genre G WHERE E.titre NOT LIKE '%hebdomadaire%' AND E.date_debut <= :d_f AND E.date_fin > :d_d AND E.no IN (".$tab_info_agenda[0]['liste_evenement_complete'].") AND E.no NOT IN (".$liste_evt.") AND V.id = E.no_ville AND T.no_ville = E.no_ville AND T.no_communautecommune = C.no AND E.no_genre = G.no";
...
$requete_liste_agenda = "SELECT E.no, E.titre, E.url_image, E.date_debut, V.nom_ville, G.libelle, V.nom_ville_url, E.no_ville, E.apparition_lettre FROM evenement E, 
    ...
    AND C.territoires_id = :t AND E.no_genre = G.no".$compl_req;
$res_liste_agenda = $connexion->prepare($requete_liste_agenda);
$res_liste_agenda->execute(array(":d_d"=>$date_debut_lettre,":d_f"=>$date_fin_lettre, ":t" => $territoire));
$tab_liste_agenda1 = $res_liste_agenda->fetchAll();

if (sizeof($tab_info_agenda) > 0) {
    $tab_liste_agenda = array_merge($tab_liste_agenda1, $tab_liste);
}
else {
    $tab_liste_agenda = $tab_liste_agenda1;
}
...
foreach ($tab_liste_agenda as $k => $v) { 
    ...
    <div class="lettre_img" name="item_agenda" data-ref="<?= $v['no'] ?>">
        <div style="text-align: right; width: 140px; height: 20px; font-weight: bolder;">
            ...
            <a style="cursor:pointer;" name="del_item_agenda"  data-ref="<?= $v['no'] ?>">X</a>
