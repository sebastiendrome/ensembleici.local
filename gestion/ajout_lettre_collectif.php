<?php             
$requete_liste_partenaires = "SELECT * FROM collectifs WHERE territoires_id = :t AND libelle <> ''";
...
foreach ($tab_liste_partenaires as $k => $v) { 
    $taille = getimagesize($root_serveur.'img/lettreinfo/'.$v['image']);
    $largeur = $taille[0];
    $hauteur = $taille[1];
    if ($largeur > $hauteur) {
        // marge en haut
    ...
    ?>
    <div class="lettre_img" name="item_collectif" data-ref="<?= $v['id'] ?>">
        <div style="text-align: right; width: 140px; height: 20px; font-weight: bolder;">
            <a style="cursor:pointer;" name="del_item_collectif"  data-ref="<?= $v['id'] ?>">X</a>
        </div>
        <?php if ($v['image'] != '') { ?>
        <div style="width: 140px; height: 140px; margin-left: 10px;"><img style="max-height: 140px; max-width: 140px; margin-left: <?= $margin_left ?>px; margin-top: <?= $margin_top ?>px" src="<?= $root_site.'img/lettreinfo/'.$v['image'] ?>" /></div>
        <?php } else { ?>
