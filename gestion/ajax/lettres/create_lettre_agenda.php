<?php
$requete_liste_agenda = "SELECT liste_evenement_valide FROM lettreinfo_agenda WHERE no_lettre=:no_l";
$res_liste_agenda = $connexion->prepare($requete_liste_agenda);
$res_liste_agenda->execute(array(":no_l" => $no));
$tab_liste_agenda = $res_liste_agenda->fetch();
$liste_valide = $tab_liste_agenda["liste_evenement_valide"];
if($liste_valide != ""){
    $requete_agenda = "SELECT E.no AS evt_no_evt,E.url_image,E.titre,E.sous_titre,E.date_debut,E.date_fin, E.no_ville AS evt_no_ville, genre.libelle AS genre, 
        ...
        IFNULL(communautecommune.libelle,'[]') AS lib_cc, communautecommune.territoires_id as territoire FROM evenement E JOIN villes ON villes.id=E.no_ville JOIN genre ON genre.no=E.no_genre 
        ...
        WHERE E.no IN (".$liste_valide.") ORDER BY territoire, lib_cc,E.date_debut,E.date_fin,villes.nom_ville_maj";
    ...
    foreach ($liste_agenda as $k => $v) {
        if ($v['territoire'] != $territoire) {
            array_push($liste_agenda, $v);
            unset($liste_agenda[$k]);
        }
    }
}
?>
...
//                echo "<a href=\"".$lien_marches."\" style=\"text-decoration:none;\" target=\"_blank\"><img src=\"".$root_site."img/lettreinfo/voir_marches.jpg\" alt=\"Voir les marchés\" width=\"266px\" style=\"width:266px;margin:0;\" /></a>";
                ...
                echo "<a href=\"".$lien_ajouter."\" style=\"text-decoration:none;\" target=\"_blank\"><img src=\"".$root_site."img/lettreinfo/ajouter_evenement.jpg\" alt=\"Ajouter un événement\" /></a>";
            ?>
            </td>
    </tr>
</table>
