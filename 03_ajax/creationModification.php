...
//if(true){
	...
	if(!empty($_POST["type"])&&($_POST["type"]=="evenement"||$_POST["type"]=="editorial"||$_POST["type"]=="forum"||$_POST["type"]=="structure"||$_POST["type"]=="petite-annonce")&&!empty($_POST["no"])){
		...
		if((!empty($_POST["no_utilisateur_creation"])) || (!empty($_SESSION["utilisateur"]["no"]))){ //modification max les utilisateurs lambda non pas de droit et on a pas toujours no_utilisateur_creation
			$reponse = array(true);
			...
			if(isset($_POST["description"])&&empty($_POST["description"])){
				$reponse = array(false,"Vous devez renseigner le champ description");
			}
			...
			else if(isset($_POST["date_fin"])&&!empty($_POST["date_fin"])&&!preg_match($regex_date,datesql($_POST["date_fin"]))){
				$reponse = array(false,"Vous devez saisir une date de fin dans un format valide (jj/mm/aaaa)");
			}
			else if(isset($_POST["date_fin"])&&!empty($_POST["date_fin"])&&date("Y-m-d")>datesql($_POST["date_fin"])){
			...
			else if(!empty($_POST["date_debut"])&&!empty($_POST["date_fin"])&&datesql($_POST["date_debut"])>datesql($_POST["date_fin"])){
				$reponse = array(false,"L'événement ne peut pas se terminer avant d'avoir commencé!");
			}
			else{
				//2.2 Vérification du format
				if(!empty($_POST["site"])){
					if(substr($_POST["site"],0,7)!="http://"&&substr($_POST["site"],0,8)!="https://")
						$_POST["site"] = "http://".$_POST["site"];
					if(!filter_var($_POST["site"], FILTER_VALIDATE_URL))
						$reponse = array(false,"l'url n'est pas valide");
				}
				if(!empty($_POST["facebook"])){
                                        if(substr($_POST["facebook"],0,7)!="http://"&&substr($_POST["facebook"],0,8)!="https://")
				...
				if(!empty($_POST["prix"])){
					if(isset($_POST["monetaire"])&&$_POST["monetaire"]==1){
						$_POST["prix"] = str_replace(",",".",$_POST["prix"]);
						$_POST["prix"] = str_replace($regex_formate_prix,"",$_POST["prix"]);
				...
				if(!empty($_POST["telephone"]))
					$_POST["telephone"] = preg_replace($regex_formate_telephone,'',$_POST["telephone"]);
				if(!empty($_POST["telephone2"]))
					$_POST["telephone2"] = preg_replace($regex_formate_telephone,'',$_POST["telephone2"]);
				if(!empty($_POST["mobile"]))
					$_POST["mobile"] = preg_replace($regex_formate_telephone,'',$_POST["mobile"]);
				...
				if(!empty($_POST["url_image"])){
					if($_POST["url_image"]!="img/logo-ensembleici_fb.jpg"){
				...
				if($reponse[0]){
					if($_POST["no"]!=-1){ //UPDATE
						...
						//On abonne l'utilisateur à son contenu
							$requete_notification = "SELECT ".$nomTablePrincipale."_notification.etat FROM ".$nomTablePrincipale."_notification WHERE ".$nomTablePrincipale."_notification.no_utilisateur=:no_utilisateur AND ".$nomTablePrincipale."_notification.no_".$nomTablePrincipale."=:no";
							$tab_notification = execute_requete($requete_notification,array(":no"=>$no_item,":no_utilisateur"=>$_SESSION["utilisateur"]["no"]));
							if(empty($tab_notification)){
								$requete_notification = "INSERT INTO ".$nomTablePrincipale."_notification(no_".$nomTablePrincipale.",no_utilisateur,etat,date_creation) VALUES(:no,:no_utilisateur,1,:date)";
								execute_requete($requete_notification,array(":no"=>$no_item,":no_utilisateur"=>$_SESSION["utilisateur"]["no"],":date"=>date("Y-m-d H:i:s")));
							}
						...
						if(isset($_POST["contact_no_0"])){
							//On insère maintenant les contacts
							//1. On supprime tous les contacts déjà existants pour l'item
							$del_contact_item = "DELETE FROM ".$nomTablePrincipale."_contact WHERE ".$nomTablePrincipale."_contact.no_".$nomTablePrincipale."=:no";
							execute_requete($del_contact_item,array(":no"=>$no_item));
							//2. On ajoute les contacts tant qu'il y en a
							$no_contact = 0;
							$requete_insert = "";
							while(isset($_POST["contact_no_".$no_contact])&&!empty($_POST["contact_no_".$no_contact])){
								//On insère dans la table correspondante no_contact, no_item, et l'éventuel no_role
								$param_insert_contact_item = array(":no"=>$no_item,":no_contact"=>$_POST["contact_no_".$no_contact]);
								if(empty($_POST["contact_no_role_".$no_contact])){
									$insert_contact_item = "INSERT INTO ".$nomTablePrincipale."_contact(no_".$nomTablePrincipale.",no_contact) VALUES(:no,:no_contact)";
								}
								else{
									$insert_contact_item = "INSERT INTO ".$nomTablePrincipale."_contact(no_".$nomTablePrincipale.",no_contact,no_role) VALUES(:no,:no_contact,:no_role)";
									$param_insert_contact_item[":no_role"] = $_POST["contact_no_role_".$no_contact];
								}
								execute_requete($insert_contact_item,$param_insert_contact_item);
								$no_contact++;
							}
						}
						
						// a corriger pour eviter le vidage continuel mais empeche de mettre 0 tag MAX
						/*else{
							execute_requete($requete_del_tag,array(":no"=>$_POST["no"]));
						}*/
						//TODO
						if(!empty($email_contact)||!empty($telephone_contact)||!empty($telephone2_contact)){
						...
						if((!empty($chaine_update)&&execute_requete($requete_update,$params)>0)||$_POST["tags"]!=$liste_tags){ //On test si l'update affecte une ligne TODO Ajouter ||$_POST["tags"]!=les_tags
					...
					//On vérifie maintenant que l'utilisateur est abonné à la fiche
					$requete_notification = "SELECT ".$nomTablePrincipale."_notification.date_modification FROM ".$nomTablePrincipale."_notification WHERE ".$nomTablePrincipale."_notification.no_utilisateur=:no_utilisateur AND ".$nomTablePrincipale."_notification.no_".$nomTablePrincipale."=:no";
					...
					if(empty($tab_notification)){ //On insère alors l'entrée
						$requete_insert = "INSERT INTO ".$nomTablePrincipale."_notification(no_".$nomTablePrincipale.",no_utilisateur,etat,date_creation) VALUES(:no,:no_utilisateur,1,:date)";
						execute_requete($requete_insert,array(":no"=>$no_item,":no_utilisateur"=>$_SESSION["utilisateur"]["no"],":date"=>date("Y-m-d H:i:s")));
					}
