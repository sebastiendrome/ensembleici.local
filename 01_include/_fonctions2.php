...
function extraire_liste($type,$limite=0,$page=1,$conditions=array()){
	...
	**/
	...
	if(!empty($conditions["no"])){ //FICHE
		...
		if($conditions["distance"]!=-1){ //-1 = tous
			if($conditions["distance"]>0){ //Rayon en kilomètre
				...
				//if($type=="forum"||!empty($conditions["tags"]))
				//	$WHERE_COUNT_PROCHE .= " GROUP BY ".$tablePrincipale.".no";
				if($type=="forum"||!empty($conditions["tags"]))
	...
	// REQUETE COUNT VILLE
	$REQUETE_COUNT_VILLE = "SELECT COUNT(".$tablePrincipale.".no) AS nb".$FROM_JOIN.((!empty($WHERE_COUNT_VILLE))?(" WHERE ".$WHERE_COUNT_VILLE):'').$GROUP_BY_VILLE;
	$count_ville = execute_requete($REQUETE_COUNT_VILLE,$param_count_ville);
	$count_ville = $count_ville[0]["nb"];
	
	// REQUETE COUNT TOTAL
	$REQUETE_COUNT_TOTAL = "SELECT COUNT(".$tablePrincipale.".no) AS nb".$FROM_JOIN.((!empty($WHERE_COUNT_TOTAL))?" WHERE ".$WHERE_COUNT_TOTAL:"").$GROUP_BY_TOTAL;
	...
	if(!empty($WHERE_COUNT_PROCHE)){
		...
		$REQUETE_COUNT_PROCHE = "SELECT COUNT(".$tablePrincipale.".no) AS nb".$FROM_JOIN.((!empty($WHERE_COUNT_PROCHE))?(" WHERE ".$WHERE_COUNT_PROCHE):'').$GROUP_BY_PROCHE;
...
function extraire_fichiers_audio_edito($no){
	$requete = "SELECT fichieraudio.no, fichieraudio.url, fichieraudio.titre, fichieraudio.auteur FROM fichieraudio
...
function connexion($email,$mdp,$admin=false){
	if(!empty($email)&&!empty($mdp)){
		...
		$requete = "SELECT utilisateur.no,utilisateur.email,utilisateur.pseudo,utilisateur.no_ville,IF(utilisateur.mot_de_passe=:mdp,1,0) AS bon_mdp, droit.no AS no_droit, droit.libelle AS libelle_droit FROM utilisateur LEFT JOIN droit_utilisateur ON droit_utilisateur.no_utilisateur=utilisateur.no LEFT JOIN droit ON droit_utilisateur.no_droit=droit.no WHERE utilisateur.etat=1 AND utilisateur.email=:email";
...
function get_bloc_publicite($nb){
	...
	$requete = "SELECT no, titre, contenu, url_image, site FROM publicites WHERE etat=1 AND validite_du<=NOW() AND validite_au>=NOW() ORDER BY RAND() LIMIT ".$nb;
