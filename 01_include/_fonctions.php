...
function extraire_fiche($type,$no){
	...
	if(count($tab)>0){
		...
		if($type=="editorial"){
            ...
            // recherche des fiches liées
            if (($type != "editorial") && ($type != "forum")) {
                $return['liaisons'] = extraire_liaisons($type, $no);
            }
	}
	return $return;
}

function extraire_liaisons($type, $no) {
//    $liste = array("Structure : Aventic", "Evenement : tartempion");
        $param = array(":no" => $no);
        $tab = array();
	if (($type == "agenda") || ($type == "evenement")) {
            // recherche dans les tables evenement_evenement, evenement_structure, evenement_petiteannonce
            $req_ev_ev = "SELECT E.titre as titre, E.no, 'Evénement' as libelle, 'evenement_evenement' as tablez FROM evenement_evenement V, evenement E WHERE E.no = V.no1 AND V.no2 = :no";
            $tab1 = execute_requete($req_ev_ev,$param);
            
            $req_ev_ev2 = "SELECT E.titre as titre, E.no, 'Evénement' as libelle, 'evenement_evenement' as tablez FROM evenement_evenement V, evenement E WHERE E.no = V.no2 AND V.no1 = :no";
            $tab2 = execute_requete($req_ev_ev2,$param);
            
            $req_ev_str = "SELECT S.nom as titre, S.no, 'Structure' as libelle, 'evenement_structure' as tablez FROM evenement_structure V, structure S WHERE S.no = V.no_structure AND V.no_evenement = :no";
            $tab3 = execute_requete($req_ev_str,$param);
            
            $req_ev_pa = "SELECT P.titre as titre, P.no, 'Annonce' as libelle, 'evenement_petiteannonce' as tablez FROM evenement_petiteannonce V, petiteannonce P WHERE P.no = V.no_petiteannonce AND V.no_evenement = :no";
            $tab4 = execute_requete($req_ev_pa,$param);
            
            $tab = array_merge($tab1, $tab2, $tab3, $tab4);
            
	}
	else if ($type == "petite-annonce"){
            // recherche dans les tables petiteannonce_petiteannonce, petiteannonce_structure, evenement_petiteannonce
            $req_pa_pa = "SELECT E.titre as titre, E.no, 'Annonce' as libelle, 'petiteannonce_petiteannonce' as tablez FROM petiteannonce_petiteannonce V, petiteannonce E WHERE E.no = V.no1 AND V.no2 = :no";
            $tab1 = execute_requete($req_pa_pa,$param);
            
            $req_pa_pa2 = "SELECT E.titre as titre, E.no, 'Annonce' as libelle, 'petiteannonce_petiteannonce' as tablez FROM petiteannonce_petiteannonce V, petiteannonce E WHERE E.no = V.no2 AND V.no1 = :no";
            $tab2 = execute_requete($req_pa_pa2,$param);
            
            $req_ev_str = "SELECT S.nom as titre, S.no, 'Structure' as libelle, 'petiteannonce_structure' as tablez FROM petiteannonce_structure V, structure S WHERE S.no = V.no_structure AND V.no_petiteannonce = :no";
            $tab3 = execute_requete($req_ev_str,$param);
            
            $req_ev_pa = "SELECT P.titre as titre, P.no, 'Evénement' as libelle, 'evenement_petiteannonce' as tablez FROM evenement_petiteannonce V, evenement P WHERE P.no = V.no_evenement AND V.no_petiteannonce = :no";
            $tab4 = execute_requete($req_ev_pa,$param);
            
            $tab = array_merge($tab1, $tab2, $tab3, $tab4);
	}
	else if (($type == "repertoire") || ($type == "structure")) {
            // recherche dans les tables structure_structure, petiteannonce_structure, evenement_structure
            $req_str_str = "SELECT E.nom as titre, E.no, 'Structure' as libelle, 'structure_structure' as tablez FROM structure_structure V, structure E WHERE E.no = V.no1 AND V.no2 = :no";
            $tab1 = execute_requete($req_str_str,$param);
            
            $req_str_str2 = "SELECT E.nom as titre, E.no, 'Structure' as libelle, 'structure_structure' as tablez FROM structure_structure V, structure E WHERE E.no = V.no2 AND V.no1 = :no";
            $tab2 = execute_requete($req_str_str2,$param);
            
            $req_ev_str = "SELECT S.titre as titre, S.no, 'Structure' as libelle, 'petiteannonce_structure' as tablez FROM petiteannonce_structure V, petiteannonce S WHERE S.no = V.no_petiteannonce AND V.no_structure = :no";
            $tab3 = execute_requete($req_ev_str,$param);
            
            $req_ev_pa = "SELECT P.titre as titre, P.no, 'Evénement' as libelle, 'evenement_structure' as tablez FROM evenement_structure V, evenement P WHERE P.no = V.no_evenement AND V.no_structure = :no";
            $tab4 = execute_requete($req_ev_pa,$param);
    ...
    if (sizeof($tab) > 0) {
        foreach ($tab as $k => $v) {
            array_push($liste, $v['libelle'].' : '.$v['titre'].' (<a name="delete_liaison" data-ref="'.$v['no'].'" data-table="'.$v['tablez'].'" style="cursor:pointer;">supprimer</a>)');
        }
    }
    return $liste;
}
...
function get_ficheLiaisons($table,$no){
    $liaisons1 = array(); $liaisons2 = array(); $liaisons3 = array(); $liaisons4 = array();
    if ($table == 'evenement') {
        $req1 = "SELECT no1 as no, '&Eacute;vénement' as type, E.titre, 'agenda' as dest FROM evenement_evenement A, evenement E WHERE no2 = :no AND A.no1 = E.no"; 
        $liaisons1 = execute_requete($req1,array(":no"=>$no));
        
        $req2 = "SELECT no2 as no, '&Eacute;vénement' as type, E.titre, 'agenda' as dest FROM evenement_evenement A, evenement E WHERE no1 = :no AND A.no2 = E.no"; 
        $liaisons2 = execute_requete($req2,array(":no"=>$no));
        
        $req3 = "SELECT no_petiteannonce as no, 'Annonce' as type, E.titre, 'petite-annonce' as dest FROM evenement_petiteannonce A, petiteannonce E WHERE A.no_evenement = :no AND A.no_petiteannonce = E.no"; 
        $liaisons3 = execute_requete($req3,array(":no"=>$no));
        
        $req4 = "SELECT no_structure as no, 'Structure' as type, E.nom as titre, 'structure' as dest FROM evenement_structure A, structure E WHERE A.no_evenement = :no AND A.no_structure = E.no"; 
        $liaisons4 = execute_requete($req4,array(":no"=>$no));
    }
    
    if ($table == 'petiteannonce') {
        $req1 = "SELECT no1 as no, 'Annonce' as type, E.titre, 'petite-annonce' as dest FROM petiteannonce_petiteannonce A, petiteannonce E WHERE no2 = :no AND A.no1 = E.no"; 
        $liaisons1 = execute_requete($req1,array(":no"=>$no));
        
        $req2 = "SELECT no2 as no, 'Annonce' as type, E.titre, 'petite-annonce' as dest FROM petiteannonce_petiteannonce A, petiteannonce E WHERE no1 = :no AND A.no2 = E.no"; 
        $liaisons2 = execute_requete($req2,array(":no"=>$no));
        
        $req3 = "SELECT no_evenement as no, '&Eacute;vénement' as type, E.titre, 'agenda' as dest FROM evenement_petiteannonce A, evenement E WHERE A.no_petiteannonce = :no AND A.no_evenement = E.no"; 
        $liaisons3 = execute_requete($req3,array(":no"=>$no));
        
        $req4 = "SELECT no_structure as no, 'Structure' as type, E.nom as titre, 'structure' as dest FROM petiteannonce_structure A, structure E WHERE A.no_petiteannonce = :no AND A.no_structure = E.no"; 
        $liaisons4 = execute_requete($req4,array(":no"=>$no));
    }
    
    if ($table == 'structure') {
        $req1 = "SELECT no1 as no, 'Structure' as type, E.nom as titre, 'structure' as dest FROM structure_structure A, structure E WHERE no2 = :no AND A.no1 = E.no"; 
        $liaisons1 = execute_requete($req1,array(":no"=>$no));
        
        $req2 = "SELECT no2 as no, 'Structure' as type, E.nom as titre, 'structure' as dest FROM structure_structure A, structure E WHERE no1 = :no AND A.no2 = E.no"; 
        $liaisons2 = execute_requete($req2,array(":no"=>$no));
        
        $req3 = "SELECT no_evenement as no, '&Eacute;vénement' as type, E.titre, 'agenda' as dest FROM evenement_structure A, evenement E WHERE A.no_structure = :no AND A.no_evenement = E.no"; 
        $liaisons3 = execute_requete($req3,array(":no"=>$no));
        
        $req4 = "SELECT no_petiteannonce as no, 'Annonce' as type, E.titre, 'petite-annonce' as dest FROM petiteannonce_structure A, petiteannonce E WHERE A.no_structure = :no AND A.no_petiteannonce = E.no"; 
        $liaisons4 = execute_requete($req4,array(":no"=>$no));
    }
    
    $liaisons = array_merge($liaisons1, $liaisons2, $liaisons3, $liaisons4);
    return $liaisons;
}
		...
		if(count($tab_requete)>0){ //Compte existant (email)
			...
			if($connect["bon_mdp"]>0){ //Le mot de passe est correct
				...
				$_SESSION["utilisateur"]["no_ville"] = $connect["no_ville"];
                                ...
                                $default_url_don = "https://www.helloasso.com/associations/association-decor/formulaires/2"; 
                                $default_ur_adhesion = "https://www.helloasso.com/associations/association-decor/adhesions/adhesion-a-l-association-decor";
                                if ($connect["territoires_id"] != '') {
                                    ...
                                    $default_url_don = $connect["url_don"];
                                    $default_url_adhesion = $connect["url_adhesion"];
                                }
                                ...
                                $_SESSION["utilisateur"]["url_don"] = $default_url_don;
                                $_SESSION["utilisateur"]["url_adhesion"] = $default_url_adhesion;
				...
				return array(false,"Mot de passe incorrect, vous pouvez le réinitialiser en cliquant sur \"Mot de passe oublié\"");
			}
		}
		else{
...
function contenu_colonne_droite($chaine){ //Chaine correspond soit à un groupe don|publicite(x)|commentaire|signaler|tag|contenu_bloc[x] soit au nom d'un groupe "pré-rempli" ci-dessous
	...
	$groupe_param = array(	"liste"=>"tag|commentaire|publicite(1)|widgets|vente(1)",
							"fiche"=>"contenu_bloc[1]|signaler|publicite(1)|vente(1)",
							"recherche"=>"contenu_bloc[1]|commentaire|publicite(1)|vente(1)",
	...
	$reg_publicite = "#publicite\(([0-9]+)\)#i";
        $reg_vente = "#vente\(([0-9]+)\)#i";
	$reg_contenu_bloc = "#contenu_bloc\[([0-9]+)\]#i";
        if (($chaine == 'fiche') && ($PAGE_COURANTE=="agenda"||$PAGE_COURANTE=="evenement")) {
            $return = get_bloc_widgets();
        } else {
            $return = '';
        }
		...
		//$return .= '<div>';
			...
			/*else if($param[$i]=="don"){
				...
				$return .= get_contenu_bloc($no_contenu_bloc[1]);
                        ...
                        else if(preg_match($reg_vente,$param[$i],$nb_vente)){
				$return .= get_bloc_vente($nb_vente[1]);
			}
		//$return .= '</div>';
	}
...
//		$return .= '<div class="libelle">Filtres<span id="show_tags" class="ico-down">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span></div>';
                $return .= '<div class="libelle">Filtres&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span id="show_tags" class="ico-plus-down" style="font-size:20px; font-weight:bolder;cursor:pointer;">+</span></div>';
...
//		$return .= '</div>';
		...
		$return .= '<div id="liste_tag" style="display:none;">';
...
function get_bloc_publicite($nb){
	...
	$requete = "SELECT no, titre, contenu, url_image, site FROM publicites WHERE etat=1 AND type IN (1,3) AND vente = 0 AND substring(validite_du, 1, 10) <= CURDATE() AND substring(validite_au, 1, 10) >= CURDATE() AND territoires_id = ".$territoire." ORDER BY RAND() LIMIT ".$nb;
...
function get_bloc_vente($nb){
        $territoire = 1;
        global $PAGE_COURANTE;
        $liste_page = '(0';
        switch ($PAGE_COURANTE) {
            case 'editorial' : $liste_page .= ',1'; break;
            case 'agenda' : $liste_page .= ',2'; break;
            case 'petite-annonce' : $liste_page .= ',3'; break;
            case 'structure' : $liste_page .= ',4'; break;
            case 'forum' : $liste_page .= ',5'; break;
            default:  break;
        }
        $liste_page .= ')';
        if (isset($_SESSION["utilisateur"]["territoire"])) {
	...
	$requete = "SELECT no, titre, contenu, url_image, site FROM publicites WHERE etat=1 AND type IN (1,3) AND vente = 1 AND validite_du<=NOW() AND validite_au>=NOW() AND page IN ".$liste_page." AND territoires_id = ".$territoire." ORDER BY page DESC, no DESC LIMIT ".$nb;
	$tab_pub = execute_requete($requete);
	$return = '';
	for($i=0;$i<count($tab_pub);$i++){
		$return .= '<div class="bloc_colonne_droite">';
			$return .= '<a href="'.$tab_pub[$i]["site"].'">';
				$return .= '<h4>'.$tab_pub[$i]["titre"].'</h4>';
				$return .= '<img src="'.$tab_pub[$i]["url_image"].'" />';
				if(!empty($tab_pub[$i]["contenu"]))
					$return .= '<p>'.$tab_pub[$i]["contenu"].'</p>';
			$return .= '</a>';
...
function get_bloc_widgets(){
        $territoire = 1;
        if (isset($_SESSION["utilisateur"]["territoire"])) {
            $territoire = $_SESSION["utilisateur"]["territoire"];
        }
	$requete = "SELECT titre, contenu FROM widgets WHERE etat=1 AND territoires_id = ".$territoire." ORDER BY RAND()";
	$tab_widg = execute_requete($requete);
	for($i=0;$i<count($tab_widg);$i++){
		$return .= '<div class="bloc_colonne_droite">';
//				$return .= '<h4>'.$tab_widg[$i]["titre"].'</h4>';
                                $return .= '<p>'.$tab_widg[$i]["contenu"].'</p>';
		$return .= '</div>';
	}
	return $return;
}
