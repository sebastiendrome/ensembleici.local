...
function select_evenements($id_ville,$nb_aff=4,$du=false,$au=false){
	...
	//2. On créait la requête à partir de ces paramètres
		...
		$where = "WHERE E.no_ville=:id_ville AND E.url_image<>'' AND E.date_fin<=DATE_ADD(E.date_debut, INTERVAL 31 DAY) AND E.date_fin>=:du".(($au!=false)?" AND E.date_debut<=:au":"");
		$order = "ORDER BY RAND()";
...
function afficher_evenement_accueil($tab_evenements){
	...
	for($i=0;$i<count($tab_evenements);$i++){
		...
		if($tab_evenements[$i]["date_debut"]!=$tab_evenements[$i]["date_fin"]){
			$tab_evenements[$i]["date"] = datehome($tab_evenements[$i]["date_debut"]).'<div class="jr">&rsaquo;</div>'.datehome($tab_evenements[$i]["date_fin"]);
			$date_du_au = "Du ".datefr($tab_evenements[$i]["date_debut"])." au ".datefr($tab_evenements[$i]["date_fin"]);
		}
		else{
			$tab_evenements[$i]["date"] = datehome($tab_evenements[$i]["date_debut"]);
			$date_du_au = "Le ".datefr($tab_evenements[$i]["date_debut"]);
		}
		if(strlen($tab_evenements[$i]["titre"])>30)
			$tab_evenements[$i]["titre"] = substr($tab_evenements[$i]["titre"],0,30)."[...]";
		$les_contenus[] = "<div class='image 4/3' style='width:100%'><img src='".$tab_evenements[$i]["url_image"]."' /><div class='home_agenda_bloc_date".$class_double."'>".$tab_evenements[$i]["date"]."</div><div class='home_agenda_bloc_description'><div>".$date_du_au." : ".$tab_evenements[$i]["titre"]." - ".$tab_evenements[$i]["libelle_genre"]."</div></div></div>";
	}
